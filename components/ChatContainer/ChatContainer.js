...
export default function ChatContainer({ onCreateUsername, username, selectedRoom, onCreateRoom }) {
  ...
  let createRoomModalUI = (
    <CreateRoomModal onCloseCreateRoomModal={onCloseCreateRoomModal} onCreateRoom={onCreateRoom} />
  );

  let headerUI = (
    <header className="text-center">
      <h1 className="display-4">Chit-Chat</h1>
      <p className="lead mb-2">Collaborate with developers worldwide to solve a bigger problem.</p>
      <button type="button" className="btn btn-primary" onClick={onShowCreateRoomModal}>Create Room</button>
    </header>
  )
  return (
    <>
